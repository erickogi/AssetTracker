package com.assettrack.assettrack.Kogi.TextView;
...
public class KogiSlantText extends View {
    ...
    public KogiSlantText(Context context) {
        this(context, null);
    }

    public KogiSlantText(Context context, AttributeSet attrs) {
        this(context, attrs, -1);
    }

    public KogiSlantText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public KogiSlantText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    public void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.KogiSlantText);

        mTextSize = array.getDimension(R.styleable.KogiSlantText_slantedTextSize, mTextSize);
        mTextColor = array.getColor(R.styleable.KogiSlantText_slantedTextColor, mTextColor);
        mSlantedLength = array.getDimension(R.styleable.KogiSlantText_slantedLength, mSlantedLength);
        mSlantedBackgroundColor = array.getColor(R.styleable.KogiSlantText_slantedBackgroundColor, mSlantedBackgroundColor);

        if (array.hasValue(R.styleable.KogiSlantText_slantedText)) {
            mSlantedText = array.getString(R.styleable.KogiSlantText_slantedText);
        }

        if (array.hasValue(R.styleable.KogiSlantText_slantedMode)) {
            mMode = array.getInt(R.styleable.KogiSlantText_slantedMode, 0);
        }
        array.recycle();
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (w != h) throw new IllegalStateException("KogiSlantText's width must equal to height");
    ...
    public KogiSlantText setText(String str) {
        mSlantedText = str;
        postInvalidate();
        return this;
    }
    ...
    public KogiSlantText setText(int res) {
    ...
    public KogiSlantText setSlantedBackgroundColor(int color) {
    ...
    public KogiSlantText setTextColor(int color) {
        mTextColor = color;
        mTextPaint.setColor(mTextColor);
        postInvalidate();
        return this;
    ...
    /**
     * @param mode :
     *             KogiSlantText.MODE_LEFT : top left
     *             KogiSlantText.MODE_RIGHT :top right
     * @return this
     */
    public KogiSlantText setMode(int mode) {
    ...
    public KogiSlantText setTextSize(int size) {
        this.mTextSize = size;
     ...
     * @return this
     */
    public KogiSlantText setSlantedLength(int length) {
